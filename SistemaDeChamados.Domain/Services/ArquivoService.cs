...
namespace SistemaDeChamados.Domain.Services
{
    public class ArquivoService : ServiceBase<Arquivo>, IArquivoService
    {
        ...
        public async Task SalvarArquivosFisicos(IList<Arquivo> arquivosStreams)
        {
            var chamadoId = arquivosStreams.Select(x => x.ChamadoId).FirstOrDefault();

            foreach (var arquivo in arquivosStreams)
            {
                using (var fileStream = File.Create(string.Format("{0}/{1}/{2}", AppDomain.CurrentDomain.BaseDirectory, chamadoId, arquivo.Filename)))
                {
